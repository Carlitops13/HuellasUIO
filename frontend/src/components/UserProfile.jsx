...
export default function UserProfile({ token, onLogout, onVolver }) {
  ...
  // Función para Eliminar Cuenta (Adoptante)
  const handleDeleteProfile = async () => {
    if (!window.confirm("¿Estás completamente seguro de que deseas eliminar tu perfil? Esta acción eliminará tu cuenta de forma permanente y es irreversible.")) {
      return;
    }
    ...
    // Usa la variable de entorno de tu .env. Si por alguna razón no lee el archivo, usa localhost por defecto.
    const baseUrl = import.meta.env.VITE_BASE_URL_PRODUCCION || "http://localhost:3000";

    try {
      const response = await fetch(`${baseUrl}/api/users/adoptante/deleteProfile`, {
        method: "DELETE",
        headers: {
      ...
      if (!response.ok) {
        throw new Error("No se pudo completar la eliminación del perfil en el servidor.");
      }
      ...
      setError(err.message || "Ocurrió un error al intentar eliminar la cuenta.");
    } finally {
      setIsLoading(false);
    }
  };
            ...
            <div className="max-w-md w-full animate-fade-in-up bg-white/50 p-8 rounded-3xl shadow-xl shadow-[#9d3d2c]/5 border border-[#ddc0bb]/30 overflow-hidden">
              ...
              {isProfileLoading ? (
                ...
                <div className="text-center animate-fade-in-up">
                  ...
                  {error && (
                    <div className="mb-4 p-2.5 bg-rose-50 text-rose-800 border border-rose-200 rounded-xl text-xs font-semibold flex items-center gap-2 text-left">
                      <span className="material-symbols-outlined text-lg text-rose-600">error_outline</span>
                      {error}
                    </div>
                  )}
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex gap-2">
                      <button onClick={() => { setIsEditingProfile(true); setError(""); setSuccess(""); }} className="w-1/2 py-2.5 bg-white border border-[#ddd9d5] text-[#1c1c19] rounded-full font-bold hover:bg-[#f7f3ee] transition-all text-xs cursor-pointer">
                        EDITAR PERFIL
                      </button>
                      <button onClick={() => { setIsEditingPassword(true); setError(""); setSuccess(""); }} className="w-1/2 py-2.5 bg-white border border-[#ddd9d5] text-[#1c1c19] rounded-full font-bold hover:bg-[#f7f3ee] transition-all text-xs cursor-pointer">
                        CLAVE
                      </button>
                    </div>
                    <button onClick={handleLogoutClick} disabled={isLoading} className="w-full py-3 bg-gradient-to-r from-[#9d3d2c] to-[#bd5541] text-white rounded-full font-bold shadow-lg shadow-[#9d3d2c]/30 hover:scale-[1.01] active:scale-[0.98] transition-all flex items-center justify-center gap-2 text-xs mt-2 cursor-pointer">
                    ...
                    {/* BOTÓN NUEVO: ELIMINAR PERFIL */}
                    <button 
                      onClick={handleDeleteProfile} 
                      disabled={isLoading}
                      className="w-full py-2.5 mt-2 text-rose-600 hover:text-rose-700 hover:bg-rose-50 border border-rose-200/60 rounded-full font-bold transition-all flex items-center justify-center gap-1.5 text-xs cursor-pointer disabled:opacity-50"
                    >
                      <span className="material-symbols-outlined text-sm">delete_forever</span>
                      {isLoading ? "PROCESANDO..." : "ELIMINAR MI PERFIL"}
                    </button>
                  </div>
                </div>
              )}
